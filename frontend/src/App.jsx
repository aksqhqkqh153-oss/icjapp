...
function BusinessCardBuilderPage() {
  ...
  function persistSnapshot(snapshot, notify = false) {
  ...
  function saveCard() {
    const hasContent = Object.values(form).some(value => String(value || '').trim()) || uploadedPhoto || uploadedPattern
    if (!hasContent) {
      window.alert('먼저 명함 정보를 입력해 주세요.')
      return
    }
    persistSnapshot(buildSnapshot(), true)
  }
        ...
        <div className="business-card-layout">
          ...
          <div className="stack business-card-form-panel">
            ...
            <div className="business-card-head-actions business-card-bottom-actions">
              <button type="button" className="ghost business-card-action-left" onClick={copySummary}>정보복사</button>
              <button type="button" className="business-card-action-right business-card-action-pc" onClick={printCard}>인쇄하기</button>
              <button type="button" className="business-card-action-right business-card-action-mobile" onClick={saveCard}>저장</button>
            </div>
          </div>
        </div>
      </section>
