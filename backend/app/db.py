...
def seed_imported_accounts(conn) -> None:
    ...
    for account in IMPORTED_ACCOUNTS:
        exists = conn.execute("SELECT id FROM users WHERE email = ?", (account['email'],)).fetchone()
        ...
        if exists:
            conn.execute(
                """
                UPDATE users SET
                    password_hash = ?, nickname = ?, role = ?, grade = ?, approved = ?, gender = ?, birth_year = ?, region = ?,
            ...
            conn.execute(
                """
                INSERT INTO users(
                    email, password_hash, nickname, role, grade, approved, gender, birth_year, region, bio, one_liner,
                    ...
                    business_address, bank_account, bank_name, mbti, google_email, resident_id, created_at
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    account['email'],
